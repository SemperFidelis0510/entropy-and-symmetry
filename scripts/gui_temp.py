...
class ImageViewer:
    def __init__(self, directory):
        ...
        self.image_files = [f for f in os.listdir(directory) if f.endswith(('.png', '.jpg', '.jpeg', '.webp'))]
    ...
    def init_window(self):
        ...
        self.image_window.bind("<F11>", self.toggle_fullscreen)
    ...
    def create_thumbnail_frame(self):
        frame_thumbnails_container = Frame(self.image_window, width=70) # 调整宽度
        frame_thumbnails_container.grid(row=0, column=0, rowspan=3, sticky="ns")
        frame_thumbnails_container.grid_propagate(False) # 禁止内部组件改变尺寸
        ...
        self.frame_thumbnails = Frame(self.canvas_thumbnails, width=60, height=len(self.List_thumbnail_images) * 60) # 增加高度
        ...
        self.canvas_thumbnails.config(scrollregion=(0, 0, 60, len(self.List_thumbnail_images) * 60)) # 调整滚动区域大小
    ...
    def load_visible_thumbnails(self, event=None):
        ...
        for idx in range(start_idx, end_idx):
            ...
            if idx not in self.loaded_thumbnails:
                ...
                self.frame_thumbnails.winfo_children()[idx].image = thumbnail_img  # 保持引用
    ...
    def forward(self, event=None):
        ...
        self.update_image()
        self.update_buttons()
        self.load_visible_thumbnails()  # 加载可见的缩略图
    ...
    def resize_image(self, percent):
        ...
        img_resized = resize(self.List_images[self.img_no], width=int(self.List_images[self.img_no].width * percent / 100))
